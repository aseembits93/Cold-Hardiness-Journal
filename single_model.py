...
def data_processing(cultivar_file, args):
    ...
    print("len(seasons)", len(valid_seasons))
    ...
    return x_train, y_train, x_test, y_test, log_dir, ferguson_train, ferguson_test
...
def training_loop(x_train, y_train, x_test, y_test, args, log_dir, device, cultivar_file):
    ...
    torch.save(model.state_dict(), os.path.join(modelSavePath, args.name,cultivar_name+ ".pt"))
    return loss_lt_10.item(), loss_lt_50.item(), loss_lt_90.item()


def evaluate(cultivar, x_test, y_test, ferguson_test, evalpath, args):
    model = net(np.array(x_test).shape[-1])
    model_filename = os.path.join('./models',evalpath,cultivar.split('/')[-1].split('.')[0]+'.pt')
    ...
    savepath = "./plots/"+evalpath+'/'+cultivar_name
    ...
    graph_residual(plot_dict, "LT10 Same Day | 2020-2021 Season |  GT - Pred",
                   days, "Days", "Temperature. (Deg. C)", cultivar_name, savepath)
    ...
    graph_residual(
        plot_dict, "LT50 Same Day | 2021 - * Season |  GT - Pred", days, "Days", "LT50", cultivar_name, savepath)
...
if __name__=='__main__':
    ...
    for cultivar in cultivars:
        x_train, y_train, x_test, y_test, log_dir, ferguson_train, ferguson_test = data_processing(args = args, cultivar_file=cultivar)
        ...
        with open(os.path.join('./models',args.name,'losses.txt'), 'a') as f:
        ...
        evaluate(cultivar, x_test, y_test, ferguson_test,args.name, args)
