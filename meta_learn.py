...
class embedding_net(nn.Module):  # LT50 and budbreak
    def __init__(self, input_size):
        ...
        self.embedding = nn.Embedding(6, 10).requires_grad_(False)
        self.combinations_weights = nn.Parameter(torch.rand(1,6))
...
def training_loop(train_data_dict, test_data_dict, log_dir, batchSize, numberOfEpochs, runID):
    ...
    print("Loading with trained model")
    model.load_state_dict(torch.load("./dgx/models/embedding_net2.pt"), strict=False)
...
def evaluate(cultivar, x_test, y_test, ferguson_test, cultivar_id, file_handle):
    ...
    model_filename = './models/embedding_metalearn.pt'
    ...
    Path("./plots/embedding_metalearn/"+cultivar_name).mkdir(parents=True, exist_ok=True)
...
cultivar_dict = {'./data/ColdHardiness_Grape_Grenache.csv':6}
...
ferguson_dict = dict()
for cultivar in ['./data/ColdHardiness_Grape_Grenache.csv']:
...
with open('metalearn_loss.txt','w') as fh:
    for i, cultivar in enumerate(['./data/ColdHardiness_Grape_Grenache.csv']):
